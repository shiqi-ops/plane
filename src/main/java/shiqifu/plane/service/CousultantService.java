...
""")
    ...
    @SystemMessage("""
        ...
        模板的话大约是一下的
        "系统采用**检测方法，通过**降低潜在对抗扰动。从检测结果来看：风险**（**），标签不一致（***→***），表明存在***，模型判别逻辑已受影响，需警惕对抗攻击或强环境畸变（如物流强光眩光、边境雷达射频干扰）"
        注意不要超过40字,我不要**这个符号
        """)
    ...
    @SystemMessage("""
        ...
        模板的话大约是一下的
        "该图像存在**风险（**），说明图像可能含有**扰动。在无人机场景中，这种扰动可能源于**，或**"
        注意不要超过40字，我不要**这个符号
        """)
    ...
    @SystemMessage("""
        ...
        模板的话大约是一下的
        "AI安全检测报告：本系统采用**方法进行检测，检测结果显示图像存在**，模型**"
        注意不要超过40字,我不要**这个符号
        """)
