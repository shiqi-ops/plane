...
def plot_attack_bubble(results):
    ...
    plt.xlabel("Query Time (Attack Speed)")
    plt.ylabel("Attack Success Rate")
    plt.title("Attack Efficiency Bubble Chart")

    plt.savefig("../results/attack_bubble.png")
    plt.close()


def calculate_score(clean,adv_list):

    avg=sum(adv_list)/len(adv_list)
    score=(avg/clean)*100
...
def generate_html(report):
    ...
    <h1>Drone Model Robustness Report</h1>
    ...
    <p>{report["clean_accuracy"]}</p>

    <h2>Attack Ranking</h2>
    <pre>{report["ranking"]}</pre>
    ...
    <h2>Attack Bubble</h2>
    <img src="attack_bubble.png" width="600">
